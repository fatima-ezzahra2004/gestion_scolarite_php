...
<div class="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition">
    ...
    <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
            <div class="flex justify-between items-start">
                ...
                <!-- MENU 3 POINTS -->
                <div class="relative">
                    <button onclick="toggleMenu(<?= (int)$p['id_prospect'] ?>)"
                            class="text-gray-400 hover:text-teal-600">
                        <i class="fa-solid fa-ellipsis text-sm"></i>
                    </button>

                    <div id="menu-<?= (int)$p['id_prospect'] ?>"
                         class="hidden absolute right-0 mt-2 w-48 bg-white border rounded-xl shadow-lg text-sm z-35">

                        <button onclick="openDetails(<?= (int)$p['id_prospect'] ?>)"
                                class="block w-full text-left px-4 py-2 hover:bg-gray-100">
                            Voir détails
                        </button>

                        <button onclick="openEdit(<?= (int)$p['id_prospect'] ?>)"
                                class="block w-full text-left px-4 py-2 hover:bg-gray-100">
                            Modifier
                        </button>
                        ...
                        <a href="convertir.php?id=<?= (int)$p['id_prospect'] ?>"
                           class="flex items-center gap-2 px-4 py-2 text-teal-600 hover:bg-teal-50">
                        ...
                        <button onclick="confirmDelete(<?= (int)$p['id_prospect'] ?>)"
                                class="block w-full text-left px-4 py-2 text-red-600 hover:bg-red-50 rounded-b-xl">
                            Supprimer
                        </button>
                    </div>
                </div>
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="convertir.php?id=<?= (int)$p['id_prospect'] ?>"
           class="text-teal-600 font-semibold text-sm flex items-center gap-1 hover:underline">
            Convertir
